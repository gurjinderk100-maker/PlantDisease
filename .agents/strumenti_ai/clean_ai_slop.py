...
NB_PATH = "notebook-2-0-faster.ipynb"

# Mappa delle sostituzioni
replacements = {
    # Emoji e frasi troppo entusiaste
    "✅ ": "",
    "🚀 ": "",
    "🧠 ": "",
    "🎉 ": "",
...
def clean_ai_slop():
    ...
    for cell in nb.get('cells', []):
        # Join lines into a single string to make replacing multiline or partial strings easier
        ...
        # Sostituzioni Regex avanzate se necessario per rimuovere emoji rimaste
        # Emoji regex (basic cleanup per emoji standard)
        source_text = re.sub(r'[\\U00010000-\\U0010ffff]', '', source_text)
        
        # Split back to lines, keeping newlines for proper JSON formatting
        lines = source_text.split('\\n')
        new_source = [line + '\\n' for line in lines[:-1]] + [lines[-1]] if lines else []
        if new_source and new_source[-1] == '\\n':
            # Edge case handling if trailing newline behavior is weird
            pass
